...
- Fallback order: the API path runs only on manual (interactive) refreshes; background polls read the statusline cache below exclusively, never touching the Keychain or the network for Claude. On a manual refresh the app tries the API first; on any API failure it falls back to the statusline cache (fresh cache → fresh data), and only then degrades to stale, surfacing the API's failure reason.
