...
- `UsageProvider` — `fetch(previous:mode:) async -> ProviderState` (with a `fetch(previous:)` convenience defaulting to `.background`). Implementations: `ClaudeUsageProvider` (via `ClaudeCredentialReading` + `HTTPTransport`, falling back to `ClaudeStatuslineCacheReading`), `CodexUsageProvider` (via `CodexCredentialReading` + `HTTPTransport`). HTTP 401 → `.tokenExpired`; other non-200/transport errors → `.networkError`; undecodable body → `.parseFailure` — always keeping last-known data. In the Claude fallback, a fresh cache wins over any API failure; when both fail the API's reason is surfaced and the cache's last-known usage is preferred over the caller's `previous`.
- `CredentialAccessMode` (`.interactive` / `.background`) — threads poller → provider → credential reader → keychain query. `.background` (the default everywhere) sets `kSecUseAuthenticationUIFail` on the `SecItemCopyMatching` query so a background poll never presents a Keychain prompt: if the item's ACL would require confirmation (e.g. Claude Code rewrote `Claude Code-credentials` on token refresh and reset its ACL), the read fails with `errSecInteractionNotAllowed` and the provider degrades to its fallback (Claude statusline cache) silently. Only `refreshNow()` uses `.interactive`, so prompts are reserved for user-initiated refreshes.
