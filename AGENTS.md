...
- `UsageProvider` — `fetch(previous:) async -> ProviderState`. Implementations: `ClaudeUsageProvider` (via `ClaudeCredentialReading` + `HTTPTransport`, falling back to `ClaudeStatuslineCacheReading`), `CodexUsageProvider` (via `CodexCredentialReading` + `HTTPTransport`). HTTP 401 → `.tokenExpired`; other non-200/transport errors → `.networkError`; undecodable body → `.parseFailure` — always keeping last-known data. In the Claude fallback, a fresh cache wins over any API failure; when both fail the API's reason is surfaced and the cache's last-known usage is preferred over the caller's `previous`.
