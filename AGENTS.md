...
The app bundle is `AIUsageBar.app` (identifier `dev.brianbell.AIUsageBar`, `LSUIElement=true` so no Dock icon), signed for personal local use — no notarization or distribution. `bundle.sh` signs ad-hoc by default; set `CODESIGN_IDENTITY` to a self-signed code-signing certificate's name so Keychain "Always Allow" grants survive rebuilds instead of re-prompting after every build. Even so, expect one keychain prompt per provider after each rebuild: keychain authorization has two layers, and while the trusted-apps ACL matches the cert-pinned designated requirement (stable across rebuilds — this is what the identity buys over ad-hoc, whose DR changes every build), the partition list matches by `teamid:` and falls back to pinning each build's cdhash when the signature carries no team identifier. A locally-created identity can never carry one: AMFI SIGKILLs at launch any binary whose signature claims a TeamIdentifier without an Apple-issued chain, and codesign only derives team ids from Apple developer certs — imitating Apple's marker extensions on a local CA does nothing (all verified empirically, July 2026; do not re-attempt `--team-identifier` with a local identity). Create the local identity with `scripts/make-signing-cert`; for fully prompt-free rebuilds, sign with a real Apple Development certificate (free personal team via Xcode) instead, whose signature carries a genuine team identifier. A bad identity fails the signing step (`codesign signing failed for identity: …`) and the staging directory is cleaned up on any exit. When rebuilding or relaunching the user's login app, do not silently accept the ad-hoc fallback: build with `CODESIGN_IDENTITY="AIUsageBar Signing" scripts/bundle.sh`, then verify with `codesign -dv AIUsageBar.app 2>&1` that `Signature=adhoc` is absent. `security find-identity -v -p codesigning` may not list this self-signed certificate as a valid identity; check for it with `security find-certificate -c "AIUsageBar Signing" -a -Z` if needed, and treat the post-build `codesign -dv` output as authoritative.
