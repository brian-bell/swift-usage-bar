...
- `CredentialAccessMode` (`.interactive` / `.background`) — threads poller → provider → credential reader → keychain query. For both providers, `.background` (the default everywhere) makes the `SecItemCopyMatching` read prompt-proof with two guards, because the Claude item can raise two different legacy prompts: `kSecUseAuthenticationUIFail` on the query suppresses the trusted-application ACL prompt (yielding `errSecInteractionNotAllowed`), and `SecKeychainSetUserInteractionAllowed(false)` before the read suppresses the partition-id password prompt that `kSecUseAuthenticationUIFail` does **not** cover (yielding `errSecAuthFailed`). Either way the read fails silently and the provider degrades (for Claude, to the statusline cache). Only `refreshNow()` uses `.interactive`, which re-enables interaction, so prompts are reserved for user-initiated refreshes.
